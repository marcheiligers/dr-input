class WordWrapInput < Input
  ...
  def handle_keyboard
    ...
    if @meta || @ctrl
      ...
      if @down_keys.include?(:a)
        ...
        index = line.wrapped? ? line.start : line.start + 1
        if @shift
          @selection_end = index
        else
        ...
        line = @lines.line_at(@selection_end)
        if @selection_end == line.end # && line.wrapped?
  ...
  def prepare_render_target
    ...
    @lines.each_with_index do |line, i|
      ...
      # CURSOR LOCATION
      # if cursor_count >= 0 && cursor_count <= line.length
      #   @cursor_y = y + @y
      #   @cursor_x = line.measure_to(cursor_count).lesser(@w) + @x
      #   cursor_count = -1
      # else
    ...
    if cursor_index == line.length && line.wrapped?
      line = @lines[line.number + 1]
      cursor_index = 0
    end
